...
## Core Mandates
1. **Parallelism**: Deploy issues in parallel by activating and calling the `tmux-orchestrator` skill. Use its horizontal pane spawning workflow (`spawn.sh`) to run up to 4 panes simultaneously. Do not execute or reference the script paths directly without first calling the skill.
...
- Call the `activate_skill` tool with the name `tmux-orchestrator` to initialize the orchestration environment.
- Ensure the state folder exists: `mkdir -p .plans/swarm-state/`
- Fire up the background aggregation daemon using the `watch-kanban.sh` script from the activated `tmux-orchestrator` skill in the background: `bash skills/tmux-orchestrator/scripts/watch-kanban.sh &`
...
- **Dispatch**: For each ready issue, ensure the `tmux-orchestrator` skill is activated and call its horizontal pane spawning helper (`spawn.sh`) to allocate it to a dedicated `tmux` pane executing a Native Skill pipeline:
