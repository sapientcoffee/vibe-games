...
*   **Web Server & Foreground Processes (No Hangs)**: DO NOT run persistent/foreground processes like `npm run dev` or `vite` directly in a task's execution pane. Doing so will block the agent's execution, causing the task to hang indefinitely and fail to complete. Instead, spawn any persistent background processes using the tmux orchestrator script: `../skills/tmux-orchestrator/scripts/spawn.sh "<command>"` (e.g., `../skills/tmux-orchestrator/scripts/spawn.sh "npm --prefix frontend run dev"`) or run them in the background (e.g., `npm run dev &`).